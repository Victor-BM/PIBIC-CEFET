...
#define PI 3.141592654 // Definindo o valor de PI
...
float aquisic(){ // Função que afere distâncias
    // -- Leitura do Sensor
    dado = 9500;
...
int zerador(){ // Função para definir o default da matriz
...
int move(){ // Função que faz com que o robô se movimente e desvie de obstáculos
...
int imprime(){ // Função para printar a matriz
    printf ("\n\n\n");
    for (i = 0; i < 26; i++){
        for (j = 0; j < 51; j++){
            printf ("%d", coord [i][j]);
